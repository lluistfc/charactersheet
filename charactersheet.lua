...
local function should_show_addon()
    ...
    if ((tonumber(player:GetMainJob()) or 0) <= 0) then return false; end
...
ashita.events.register('load', 'charactersheet_load_cb', function ()
    ...
    sheet.initialize(
        ...
        function () settings.save(); end,
        equipViewerConfig.sheet_width,
        function (width) equipViewerConfig.sheet_width = width; end,
        function () gearinfo_panel.toggle(); end);
    equipViewerConfig.gearinfo = gearinfo_panel.initialize(
        equipViewerConfig.gearinfo, function () settings.save(); end);
end);
...
ashita.events.register('command', 'charactersheet_command_cb', function (e)
    ...
    if (command == 'syncbrd') then
        ...
        if (#unused_items == 0) then
            print(chat.header(addon.name):append(chat.message(
                'Unused equippable items: none.')));
        else
            print(chat.header(addon.name):append(chat.message(
                ('Unused equippable items (%d):'):format(#unused_items))));
            for _, item in ipairs(unused_items) do
                print(chat.header(addon.name):append(chat.message(
                    ('%s [%s]'):format(item.name, item.bag))));
            end
        end
        print(chat.header(addon.name):append(chat.message(
            'BRD sync: gear export refreshed; reloading LuAshitacast profile...')));
    ...
    if (command:any('gearinfo', 'gi')) then
        ...
        if (subcommand == 'show') then gearinfo_panel.set_visible(true);
        elseif (subcommand == 'hide') then gearinfo_panel.set_visible(false);
        elseif (subcommand == 'toggle') then gearinfo_panel.toggle();
        elseif (subcommand == 'lock') then gearinfo_panel.toggle_lock();
        elseif (subcommand == 'dw') then gearinfo_panel.toggle_dw();
        elseif (subcommand == 'scale' and tonumber(args[4])) then gearinfo_panel.set_scale(args[4]);
        elseif (subcommand == 'width' and tonumber(args[4])) then gearinfo_panel.set_width(args[4]);
        elseif (subcommand == 'reset') then gearinfo_panel.reset();
        else
            print(chat.header(addon.name):append(chat.message(
                '/charactersheet gearinfo [toggle|show|hide|lock|dw|scale <n>|width <px>|reset]')));
        end
        return;
    end
...
ashita.events.register('mouse', 'charactersheet_mouse_cb', function (e)
    ...
    if (equipViewerConfig.sheet.lock_position ~= nil
            and equipViewerConfig.sheet.lock_position[1]) then
    ...
    local function hit_test(px, py)
        if (sheet.is_collapsed()) then
            ...
            return (px >= x and px <= (x + grid_size)
                and py >= y and py <= (y + grid_size));
        end
        ...
        return (px >= x and px <= (x + panel_width - 200)
            and py >= y and py <= (y + 34));
    end
...
ashita.events.register('d3d_present', 'charactersheet_present_cb', function ()
    ...
    local render_width = sheet.is_collapsed()
        and (grid_width + 28)
        or sheet.get_render_width(equipViewerConfig.sheet_width);
    equipViewerConfig.position[1] = math.max(
        8, math.min(tonumber(equipViewerConfig.position[1]) or 8, screen_width - render_width - 8));
    equipViewerConfig.position[2] = math.max(
        8, math.min(tonumber(equipViewerConfig.position[2]) or 8, screen_height - 80));
